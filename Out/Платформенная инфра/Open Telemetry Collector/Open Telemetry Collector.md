...
```mermaid
stateDiagram-v2
    state "Open Telemetry Controller" as Open_Telemetry_Controller
    state "OpenTelemetry Operator" as OpenTelemetry_Operator
    state "Grafana Tempo" as Grafana_Tempo
    Open_Telemetry_Controller --> Kubernetes
    Open_Telemetry_Controller --> OpenTelemetry_Operator
    Open_Telemetry_Controller --> Grafana_Tempo
    Open_Telemetry_Controller --> Prometheus
    Open_Telemetry_Controller --> OpenSearch
```
...
- **batch** — processor, который накапливает данные и отдаёт их дальше пакетами, снижая число сетевых вызовов.
- **ClusterIP** — виртуальный внутренний IP Kubernetes Service; обычный Service принимает трафик на этот адрес и распределяет соединения.
- **ConfigMap** — объект Kubernetes для несекретных настроек; секретные значения в него класть не следует.
...
- **Deployment** — контроллер Kubernetes, который держит заданное число одинаковых Pod без закрепления за конкретным узлом.
- **Distribution (дистрибутив)** — готовая сборка Collector с зафиксированным набором модулей.
- **DNS** — служба имён: превращает имя Service или Pod в IP-адреса, по которым клиент устанавливает соединение.
...
- **file_storage** — extension, которое может сохранять очередь или состояние на диск; это ограниченный буфер, а не замена бэкенда.
- **Gateway** — роль централизованного Collector с доступным клиентам endpoint.
- **gRPC** — бинарный RPC-протокол; для OTLP обычно слушает порт 4317/TCP, если этот endpoint включён.
...
- **health_check** — extension, которое публикует HTTP-проверку живости процесса для проб Kubernetes.
- **Helm** — инструмент установки по шаблону (chart); это способ развёртывания, а не часть бинарника Collector.
- **k8sattributes** — processor, который добавляет к телеметрии сведения Kubernetes: Pod, узел, namespace и подобные атрибуты.
- **Kubernetes Secret** — объект Kubernetes API для сертификатов, ключей и токенов, монтируемых в Pod.
- **Kubernetes Service** — объект, дающий стабильное имя и распределение входящих соединений по Pod; обычный Service не гарантирует маршрутизацию по Trace ID.
- **loadbalancing exporter** — exporter Collector, который направляет данные на конкретный backend-endpoint по ключу из телеметрии, а не только по случайному сетевому соединению.
- **manifest.yaml** — перечень модулей конкретного distribution; компонент существует в сборке только если он там указан.
- **memory_limiter** — processor, который ограничивает потребление памяти и при переполнении может отказывать в приёме данных.
- **OTLP** — OpenTelemetry Protocol для передачи трасс, метрик и логов по gRPC или HTTP.
- **OpenTelemetry Operator** — отдельный Kubernetes-контроллер, который создаёт и обновляет ресурсы Collector и может включать автоинструментацию; поток телеметрии через него не проходит.
...
- **Pod** — наименьшая единица запуска в Kubernetes: один или несколько контейнеров с общей сетью, в том числе процесс Collector.
...
- **Runtime-инстанс** — отдельный рабочий экземпляр компонента внутри процесса; у processor он свой для каждого pipeline.
- **Scrape** — опрос, при котором потребитель сам забирает метрики с HTTP-endpoint, а Collector их не отправляет.
- **Sending queue** — ограниченная очередь exporter перед отправкой и повторами.
- **Sidecar** — контейнер или процесс рядом с приложением в том же Pod или на той же машине; agent Collector может работать так, а не только как DaemonSet.
...
- **Span (спан)** — один шаг распределённого трейса; набор спанов с одним Trace ID составляет трейс.
- **Stateful processor** — processor, результат которого зависит от накопленного состояния и согласованности поступающего потока.
- **StatefulSet** — контроллер Kubernetes для Pod с устойчивым именем и, при необходимости, собственным томом; выбирается не по умолчанию, а если нужна стабильная идентичность.
- **Stateless processor** — processor, обрабатывающий элемент без зависимости от истории на конкретной реплике.
- **TLS** — шифрование канала между клиентом и сервером; без него OTLP передаёт телеметрию открытым текстом.
