...
Архитектура разделена на четыре группы:

1. **Control plane** — управляет состоянием кластера и принимает решения.
2. **Worker-узлы** — запускают прикладные поды.
3. **Add-ons** — расширяют кластер сетью, DNS, хранилищем, входящим трафиком и метриками.
4. **Внешние системы** — развёртываются и эксплуатируются отдельно от Kubernetes.

Control plane и worker — разные роли. Add-ons не следует называть ни control plane, ни worker-компонентами: они являются отдельным программным слоем и обычно сами запускаются как поды на нодах.
